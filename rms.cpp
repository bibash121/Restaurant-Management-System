    ...
    , ui(new Ui::RMS)
{
    ...
    ui->MenuTable->hideColumn(2);
     ...
     connect(ui->SalesTable, &QTableWidget::itemChanged, [=]() {
         updateNetTotal(ui->SalesTable,ui-> NetSalesBox);
     });
...
void RMS::on_Calculate_clicked()
{
    ...
    {
        ui->OrderErrorLabel->setText("List cannot be empty");
    }
    else{
        ui->OrderErrorLabel->clear();
...
void RMS::on_SearchDetailButton_clicked()
{
    ...
    query.bindValue(":username", searchText);
...
void RMS::onCalendarSelectionChanged()
{
    ...
    while (query.next()) {
        ...
        ui->SalesTable->setItem(row, 0, new QTableWidgetItem(query.value(0).toString()));
        ui->SalesTable->setItem(row, 1, new QTableWidgetItem(query.value(1).toString()));
        ui->SalesTable->setItem(row, 2, new QTableWidgetItem(query.value(2).toString()));
        ui->SalesTable->setItem(row, 3, new QTableWidgetItem(query.value(3).toString()));
        ui->SalesTable->setItem(row, 4, new QTableWidgetItem(query.value(4).toString()));
        row++;
    }
}
...
void RMS::on_RefreshButton_clicked()
    ...
    void RMS::updateNetTotal(QTableWidget* table, QLabel* NetSalesBox) {
        double total = 0.0;

        for (int row = 0; row < table->rowCount(); ++row) {
            bool ok;
            double value = ui->SalesTable->item(row, 4) ? ui->SalesTable->item(row, 4)->text().toDouble(&ok) : 0.0;
            if (ok) {
                total += value;
            }
        }

        ui->NetSalesBox->setText(QString::number(total, 'f', 2)); // Set the total with 2 decimal places
    }
